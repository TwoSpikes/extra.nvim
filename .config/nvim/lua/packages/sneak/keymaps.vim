...
\&& g:compatible !=# "helix_hard"
	...
	nnoremap f  <cmd>let g:lx=line('.')<bar>let g:ly=col('.')<bar>let g:sneak_mode = 'f'<bar>call sneak#wrap('',1,0,1,0)<bar>exec "normal! my"<bar>let g:visual_mode="char"<bar>let g:pseudo_visual=v:true<bar>execute "normal! `'v`y"<bar>let g:rx=line('.')<bar>let g:ry=col('.')<bar>augroup SneakDisableTCr<bar>execute "xunmap t<"."cr>"<bar>call AfterSomeEvent("CursorMoved", "call sneak#cancel()\|xnoremap t<"."cr>", {name -> "if !sneak#is_sneaking()\|au! ".name."\|endif"})<cr>
