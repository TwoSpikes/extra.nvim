...
let g:XCT_BUF_NAME = '__XterpColorTable__'
let s:bufname = g:XCT_BUF_NAME
command! Sxct call s:XtermColorTable('split')
command! Vxct call s:XtermColorTable('vsplit')
command! Txct call s:XtermColorTable('tabnew')
command! Exct call s:XtermColorTable('edit')
...
function! s:HighlightCell(n, bgf)
    let rgb = s:xterm_colors[a:n]
"dgf: -2) black or white depending on intensity
